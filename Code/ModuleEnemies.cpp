...
void ModuleEnemies::OnCollision(Collider* c1, Collider* c2) {
	for (uint i = 0; i < MAX_ENEMIES; ++i) {
		if (enemies[i] != nullptr && enemies[i]->GetCollider() == c1) {
			...
			if (enemies[i]->type == LIGHT_SHOOTER || enemies[i]->type == LIGHT_SHOOTER_LATERAL || enemies[i]->type == HOAMING_MISSILE) {
				if (enemies[i]->vida > 0)
			...
			else if (enemies[i]->type == BASIC_TANK || enemies[i]->type == TURRET || enemies[i]->type == HEAVY_TURRET) {
				if (enemies[i]->vida > 0)
					App->music->PlayFX(hit);
				else  {
...
int ModuleEnemies::hoamingReturnEnem(int enem) {
	if (enem < 0) {
		for (int i = 0; i < MAX_ENEMIES; i++) {
			if (enemies[i] != nullptr && enemies[i]->type != HOAMING_MISSILE && enemies[i]->type != MISSILE_UP && enemies[i]->type != POWER_UP && enemies[i]->type != MEDAL && enemies[i]->type != BOSS6_BASE && enemies[i]->type != BASIC_TANK_DESTROY && enemies[i]->type != HEAVY_TURRET_DESTROY && enemies[i]->position.y < SCREEN_HEIGHT) {
				return i;
			}
		}
	} else {
