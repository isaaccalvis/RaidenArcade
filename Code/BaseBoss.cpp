...
Base_Boss::Base_Boss(int x, int y) : Enemy(x, y) {
	...
	//collider = App->collision->AddCollider({ 0, 0, 86, 50 }, COLLIDER_TYPE::COLLIDER_NONE, (Module*)App->enemies);
	current_time = 0;
	dispara = false;
...
void Base_Boss::Move() {
	if (estat == 0) {
		estat = -4; // aquest estat es l'animacio inicial
		...
		if (capsBoss[0] == -1 && capsBoss[1] == -1 && estat < 2) {	// Quan es mor
			...
			if (estat >= 2 && estat < 10) {	// EXPLOSIO QUAN LA PALMA
